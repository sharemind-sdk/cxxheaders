...
#include "../src/ToUnsigned.h"

#include <cstdint>
#include <type_traits>


using sharemind::toUnsigned;

template <typename T>
struct TestWithSigned {
    static_assert(std::is_signed<T>::value, "");
    using R = decltype(toUnsigned(std::declval<T>()));
    using RR = decltype(toUnsigned(std::declval<T &>()));
    using RCR = decltype(toUnsigned(std::declval<T const &>()));
    using RVR = decltype(toUnsigned(std::declval<T volatile &>()));
    using RCVR = decltype(toUnsigned(std::declval<T const volatile &>()));
    ...
    static_assert(std::is_same<R, typename std::make_unsigned<T>::type>::value,
                  "");
